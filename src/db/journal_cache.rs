...
pub fn batch_get(
    conn: &Connection,
    names: &[String],
) -> Result<std::collections::HashMap<String, JournalRanking>> {
...
pub fn batch_upsert(conn: &Connection, rankings: &[JournalRanking]) -> Result<()> {
    ...
    conn.execute_batch("BEGIN IMMEDIATE TRANSACTION")
        .map_err(|e| {
            GscholarError::Database(format!("Begin batch upsert transaction failed: {}", e))
        })?;
    ...
    conn.execute_batch("COMMIT").map_err(|e| {
        GscholarError::Database(format!("Commit batch upsert transaction failed: {}", e))
    })?;

    debug!(
        entries = rankings.len(),
        "Journal cache batch upsert complete"
    );
    Ok(())
}
...
pub fn delete(conn: &Connection, name: &str) -> Result<bool> {
    let rows = conn
        .execute("DELETE FROM journal_cache WHERE name = ?1", params![name])
...
pub fn clear_all(conn: &Connection) -> Result<usize> {
    let rows = conn
        .execute("DELETE FROM journal_cache", [])
...
pub fn get_stats(conn: &Connection) -> Result<CacheStats> {
    ...
    let oldest_entry_at: Option<i64> = conn
        .query_row("SELECT MIN(fetched_at) FROM journal_cache", [], |row| {
            row.get(0)
        })
        .ok()
        .flatten();

    let newest_entry_at: Option<i64> = conn
        .query_row("SELECT MAX(fetched_at) FROM journal_cache", [], |row| {
            row.get(0)
        })
        .ok()
        .flatten();

    let (total_lookups, cache_hits): (i64, i64) = conn
        .query_row(
