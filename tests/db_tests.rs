...
use rscholar::db::{analytics, api_keys, journal_cache, tasks};
...
fn test_api_keys_create_and_validate() {
    ...
    let created = api_keys::create(&conn, "Test Key", false, 10).expect("Failed to create key");

    assert!(created.key.expose_secret().starts_with("rgs_"));
    assert_eq!(created.name, "Test Key");

    let validated =
        api_keys::validate(&conn, created.key.expose_secret()).expect("Failed to validate key");
...
fn test_api_keys_invalid_key() {
    ...
    let result = api_keys::validate(&conn, "invalid-key-12345").expect("Validate should not error");
...
fn test_api_keys_update() {
    ...
    let updated = api_keys::update(&conn, &created.id, Some("Updated"), Some(50)).expect("update");
    assert!(updated);
...
fn test_journal_cache_delete() {
    ...
    assert!(journal_cache::get(&conn, "ToDelete")
        .expect("get")
        .is_none());
}
